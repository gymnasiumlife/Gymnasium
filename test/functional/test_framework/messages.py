...
def gymnasiumhash(s):
    return gymnasium_hash.getPoWHash(s)
...
# Objects that map to gymnasiumd objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(gymnasiumhash(r))
            self.hash = gymnasiumhash(r)[::-1].hex()
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(gymnasiumhash(r))
            self.hash = int(gymnasiumhash(r)[::-1].hex(), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in gymnasiumd indicates these should be deserialized as blocks
