...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def gymnasium_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/gymnasium-wallet' + self.config["environment"]["EXEEXT"]
        args = ['-datadir={}'.format(self.nodes[0].datadir), '-regtest'] + list(args)
        return subprocess.Popen([binary] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)

    def assert_raises_tool_error(self, error, *args):
        p = self.gymnasium_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `gymnasium-wallet help` raises an error. Use `gymnasium-wallet -help`.
        self.assert_raises_tool_error('Invalid command: help', 'help')
        self.assert_raises_tool_error('Error: two methods provided (info and create). Only one method should be provided.', 'info', 'create')
        self.assert_raises_tool_error('Error parsing command line arguments: Invalid parameter -foo', '-foo')
        locked_dir = os.path.join(self.options.tmpdir, "node0", self.chain, "wallets")
        error = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another gymnasiumd?"
