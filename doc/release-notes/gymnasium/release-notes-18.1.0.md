Gymnasium Core version v18.1.0
=========================
...
Gymnasium Core imperfectly follows semantic versioning. Breaking changes should be
...
Support for the BIP70 Payment Protocol has been dropped from Gymnasium Qt.
...
The systemd init file (`contrib/init/gymnasiumd.service`) has been changed to use
`/var/lib/gymnasiumd` as the data directory instead of `~gymnasium/.gymnasium`. This
change makes Gymnasium Core more consistent with other services, and makes the
...
`datadir` option in `/etc/gymnasium/gymnasium.conf` will have no effect. This is
because the command line arguments specified in the init files take precedence
over the options specified in `/etc/gymnasium/gymnasium.conf`.
...
See the [release notes from v18.0.1](https://github.com/gymnasiumlife/blob/v18.0.1/doc/release-notes.md) for a full description of the changes from the
...
the deprecation of wallet accounts in Gymnasium v0.17 and introduction of PSBT
format.
...
- The `testnet` field in `gymnasium-cli -getinfo` has been renamed to `chain` and now returns the current network name as defined in BIP70 (main, test, regtest).
...
This release introduces many hundreds updates from Bitcoin v0.18/v0.19/v0.20/v0.21/v22. Bitcoin changes that do not align with Gymnasium’s product needs, such as SegWit and RBF, are excluded from our backporting. For additional detail on what’s included in Bitcoin, please refer to their release notes.

v18.1.0 Change log
==================

See detailed [set of changes](https://github.com/gymnasiumlife/compare/v18.0.2...gymnasiumpay:v18.1.0).
