# Gymnasium Core version v20.0.0
...
Currently, 10% of block rewards are set aside for the Gymnasium DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Gymnasium in a
credit pool. This credit pool mirrors the available credit supply in Gymnasium
Platform. When a certain amount of Gymnasium is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Gymnasium
Platform. Masternode identities receive the fees and can convert them back to
Gymnasium by withdrawing them from Gymnasium Platform.
...
- `bestCLHeightDiff` (`uint32`) is equal to the diff in heights from the mined
  block height
- `creditPoolBalance` (`int64`) is equal to all the GymnasiumS locked in all previous
  blocks minus all the GymnasiumS unlocked in all previous blocks plus all the block
...
### Removal of reject network messages from Gymnasium Core (BIP61)

This feature has been disabled by default since Gymnasium Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Gymnasium Core version 20.0.0 there are extra changes:
...
- It is possible to run Gymnasium Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `GSM/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Gymnasium Core (BIP61)_](#removal-of-reject-network-messages-from-gymnasium-core-bip61) above for
...
### Gymnasium-specific changes in existing RPCs:
...
- The `gymnasium-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `gymnasiumd --help` for
more information.
...
python test framework and `gymnasium-cli` tool both take advantage of this, so
for example:

```sh
gymnasium-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
gymnasium-cli -named createwallet mywallet load_on_startup=1
```
...
with Gymnasium’s product needs, such as SegWit and RBF, are excluded from our
...
[DIP-0023]: https://github.com/gymnasiumpay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/gymnasiumpay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/gymnasiumlife/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/gymnasiumlife/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/gymnasiumlife/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/gymnasiumlife/compare/v19.3.0...gymnasiumpay:v20.0.0
