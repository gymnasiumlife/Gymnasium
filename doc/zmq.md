...
The Gymnasium Core daemon can be configured to act as a trusted "border
router", implementing the gymnasium wire protocol and relay, making
...
The ZeroMQ feature in Gymnasium Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building gymnasiumd:
...
For instance:
    ...
    $ gymnasiumd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/gymnasiumd.tx.raw \
...
These options can also be provided in gymnasium.conf.
...
From the perspective of gymnasiumd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into gymnasiumd directly. Furthermore, no information is
...
using. Gymnasiumd appends an up-counting sequence number to each
