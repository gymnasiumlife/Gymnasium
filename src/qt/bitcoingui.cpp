...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsMenuAction->setStatusTip(tr("Send coins to a Gymnasium address"));
    ...
    coinJoinCoinsMenuAction->setStatusTip(tr("Send %1 funds to a Gymnasium address").arg(strCoinJoinName));
    coinJoinCoinsMenuAction->setToolTip(coinJoinCoinsMenuAction->statusTip());

    receiveCoinsMenuAction = new QAction(tr("&Receive"), this);
    receiveCoinsMenuAction->setStatusTip(tr("Request payments (generates QR codes and gymnasium: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Gymnasium addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Gymnasium addresses"));
    m_load_psbt_action = new QAction(tr("Load PSBT..."), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Gymnasium Transaction"));
    ...
    openAction->setStatusTip(tr("Open a gymnasium: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Gymnasium command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (fNetworkActive) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Gymnasium network", "", count));
    } else {
...
void BitcoinGUI::updateWidth()
{
    ...
    // Use nButtonsVisible + 1 <- for the gymnasium logo
