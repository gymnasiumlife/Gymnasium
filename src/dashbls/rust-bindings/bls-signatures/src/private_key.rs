...
impl PrivateKey {
    ...
    #[cfg(feature = "gymnasium_helpers")]
    pub fn generate_gymnasium(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "gymnasium_helpers")]
    pub fn generate_gymnasium_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_gymnasium(rng))
            .collect()
    }
