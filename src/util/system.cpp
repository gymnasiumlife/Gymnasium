...
const char * const BITCOIN_CONF_FILENAME = "gymnasium.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //gymnasium-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Gymnasium
    // macOS: ~/Library/Application Support/Gymnasium
    // Unix-like: ~/.gymnasium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gymnasium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Gymnasium";
#else
    // Unix-like
    return pathRet / ".gymnasium";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty gymnasium.conf if it does not exist
