...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 GSM irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on gymnasiumcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Gymnasium features */
    { "gymnasium",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "gymnasium",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "gymnasium",               "gobject",                &gobject,                {} },
    { "gymnasium",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
