 ...
 * database.
 *
 * CBlock(hash=00000ffd590b14, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=e0028e, nTime=1713101722, nBits=1e0ffff0, nNonce=753014, vtx=1)
...
{
    const char* pszTimestamp = "fitness crossfit spinning pilates squash zumba step aerobik basketbol";
    const CScript genesisOutputScript = CScript() << ParseHex("04e32693b1a87c3fad43696ac8999b4f2d6ec7c4e2df8b613bd917ac4650f0bed94f93b2d321827138477d862013342e1532f52a0bbbfeb988941a0507edddf618") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        consensus.nPowLWMAHeight = 46500;
        ...
        consensus.defaultAssumeValid = uint256S("0x0000016d5b75218dfc03cc5a91c4aea63764b24538928edcab26a26f0151b401"); 
        ...
        genesis = CreateGenesisBlock(1713101722, 753014, 0x1e0ffff0, 1, 5 * COIN);
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x0000016d5b75218dfc03cc5a91c4aea63764b24538928edcab26a26f0151b401"));
        assert(genesis.hashMerkleRoot == uint256S("0xa4bcfcb791b53d2c49096eae33b9901504f1d6b83404308e08e1d9506353d259"));
        ...
        vSeeds.emplace_back("node1.gymnasium.life");
        vSeeds.emplace_back("node2.gymnasium.life");
        ...
        // Gymnasium BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // Gymnasium BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xAD, 0xE4};

        // Gymnasium BIP44 coin type is '5'
        nExtCoinType = 5;
        ...
        vSporkAddresses = {"GMTca8eU5wU5HJLSDRRTnvDQ2jF8GxboFg"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x0000016d5b75218dfc03cc5a91c4aea63764b24538928edcab26a26f0151b401")},
            }
        };
        ...
        // getchaintxstats 17280 0000016d5b75218dfc03cc5a91c4aea63764b24538928edcab26a26f0151b401
        chainTxData = ChainTxData{
                1713101722, // * UNIX timestamp of last known number of transactions
...
public:
    CTestNetParams() {
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x000004a6802995b45880555996f50f2c24432bd56a0dab8c2ced9c9336216fba"); 
        ...
        genesis = CreateGenesisBlock(1713101727UL, 447895UL, 0x1e0ffff0, 1, 5 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x000004a6802995b45880555996f50f2c24432bd56a0dab8c2ced9c9336216fba"));
        assert(genesis.hashMerkleRoot == uint256S("0xa4bcfcb791b53d2c49096eae33b9901504f1d6b83404308e08e1d9506353d259"));
        ...
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,166);
        // Testnet Gymnasium BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet Gymnasium BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet Gymnasium BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
        ...
        vSporkAddresses = {"GSCWfiRwCtg2Sv5yRATR5xhqpVGVMLE9ZJ"};
        nMinSporkKeys = 1;

        checkpointData = {
            {
                {0, uint256S("0x000004a6802995b45880555996f50f2c24432bd56a0dab8c2ced9c9336216fba")},
            }
        };
        ...
        // getchaintxstats 17280 000004a6802995b45880555996f50f2c24432bd56a0dab8c2ced9c9336216fba
        chainTxData = ChainTxData{
                1713101727, // * UNIX timestamp of last known number of transactions
                0,    // * total number of transactions between genesis and that timestamp
                            //   (the tx=... number in the ChainStateFlushed debug.log lines)
                0,       // * estimated number of transactions per second after that timestamp
        };
...
public:
    explicit CDevNetParams(const ArgsManager& args) {
        ...
        genesis = CreateGenesisBlock(1713101732, 2, 0x207fffff, 1, 5 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        ...
        assert(consensus.hashGenesisBlock == uint256S("0x3bd3a9a192d9512ff902c02c4243b3f1dc236dd55c32b58670951c6d1bc1ba1d"));
        assert(genesis.hashMerkleRoot == uint256S("0xa4bcfcb791b53d2c49096eae33b9901504f1d6b83404308e08e1d9506353d259"));
        ...
        checkpointData = (CCheckpointData) {
            {
                { 0, uint256S("0x3bd3a9a192d9512ff902c02c4243b3f1dc236dd55c32b58670951c6d1bc1ba1d")},
...
public:
    explicit CRegTestParams(const ArgsManager& args) {
        ...
        // privKey: 6ZvTXwAoDJPP4oZV5YdJrvyRU6b8FLuwoEQDM4QbRkFhkxzDbGr
        vSporkAddresses = {"GaYwZXCo6ryPjs3LoMDoQw21xYPyrKp469"};
        nMinSporkKeys = 1;
        ...
        // Regtest Gymnasium addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,38);
        // Regtest Gymnasium script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,39);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,166);
        // Regtest Gymnasium BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Gymnasium BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Gymnasium BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
